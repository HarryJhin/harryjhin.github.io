...
**3. AOT 캐시(Project Leyden).** 최신 JDK의 흐름이다. JEP 483이 JDK 24에 들어오면서, 로딩·링킹된 클래스를 캐시해 재사용한다. 공개된 Spring PetClinic 측정에서 기동이 4.486초(JDK 23)에서 2.604초(JDK 24)로, 대략 40% 줄었다. JDK 25는 여기에 단일 단계 명령(`-XX:AOTCacheOutput`) 같은 사용성 개선을 더했다. GraalVM의 닫힌 세계 제약 없이 일반 JVM에서 기동을 줄이는 길이라 매력적이다.
